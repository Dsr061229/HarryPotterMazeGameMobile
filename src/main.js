...
var textureLoader=new THREE.TextureLoader(),assetTextureCache={};
function getAssetTexture(src){if(!assetTextureCache[src]){var tex=textureLoader.load(src);tex.anisotropy=Math.min(4,renderer.capabilities.getMaxAnisotropy?renderer.capabilities.getMaxAnisotropy():1);if(THREE.SRGBColorSpace)tex.colorSpace=THREE.SRGBColorSpace;assetTextureCache[src]=tex}return assetTextureCache[src]}
function makeAssetMaterial(src,opacity){return new THREE.MeshBasicMaterial({map:getAssetTexture(src),transparent:true,alphaTest:0.08,opacity:opacity===undefined?1:opacity,side:THREE.DoubleSide,depthWrite:false})}
function makeGroundShadow(rx,rz,opacity){var m=new THREE.Mesh(new THREE.CircleGeometry(1,32),new THREE.MeshBasicMaterial({color:0x050806,transparent:true,opacity:opacity||0.34,depthWrite:false}));m.rotation.x=-Math.PI/2;m.scale.set(rx,rz,1);m.position.y=0.035;return m}
function makeCreatureStandee(src,w,h,opts){opts=opts||{};var group=new THREE.Group();group.userData.animSeed=Math.random()*100;group.userData.kind=opts.kind||"creature";group.userData.baseY=opts.baseY||0;var shadow=makeGroundShadow(opts.shadowX||0.9,opts.shadowZ||0.55,opts.shadowOpacity||0.32);shadow.name="assetShadow";group.add(shadow);if(opts.core){var core=new THREE.Mesh(opts.core.geo,opts.core.mat);core.position.copy(opts.core.pos||new THREE.Vector3(0,h*0.35,0));core.scale.copy(opts.core.scale||new THREE.Vector3(1,1,1));core.castShadow=true;core.receiveShadow=true;core.name="assetCore";group.add(core)}var back=new THREE.Mesh(new THREE.PlaneGeometry(w*0.82,h*0.92),new THREE.MeshBasicMaterial({map:getAssetTexture(src),color:opts.backColor||0x050605,transparent:true,alphaTest:0.08,opacity:opts.backOpacity||0.35,side:THREE.DoubleSide,depthWrite:false}));back.position.set(0,h*0.52,-0.08);back.scale.set(1.08,1.04,1);back.name="assetBack";group.add(back);var art=new THREE.Mesh(new THREE.PlaneGeometry(w,h),makeAssetMaterial(src,opts.opacity));art.position.set(0,h*0.52,0.09);art.castShadow=true;art.name="assetArt";group.add(art);if(opts.ring){var ring=new THREE.Mesh(new THREE.TorusGeometry(opts.ring.radius,opts.ring.tube,12,48),new THREE.MeshBasicMaterial({color:opts.ring.color,transparent:true,opacity:opts.ring.opacity||0.45,side:THREE.DoubleSide,depthWrite:false}));ring.position.set(0,opts.ring.y||h*0.48,-0.12);ring.rotation.x=Math.PI/2;ring.name="assetRing";group.add(ring)}return group}
function addMistWisps(group,color,count,spreadY){var mat=new THREE.MeshBasicMaterial({color:color,transparent:true,opacity:0.28,depthWrite:false});for(var i=0;i<count;i++){var wisp=new THREE.Mesh(new THREE.SphereGeometry(0.08+Math.random()*0.09,8,6),mat.clone());wisp.position.set((Math.random()-0.5)*1.7,0.35+Math.random()*spreadY,(Math.random()-0.5)*0.7);wisp.userData={seed:Math.random()*100,home:wisp.position.clone()};wisp.name="assetWisp";group.add(wisp)}}
function animateAssetCreature(group,t,dt,state){if(!group)return;var seed=group.userData.animSeed||0,kind=group.userData.kind||"",art=group.getObjectByName("assetArt"),back=group.getObjectByName("assetBack"),shadow=group.getObjectByName("assetShadow"),ring=group.getObjectByName("assetRing");if(kind==="dementor"){var bob=Math.sin(t*2.2+seed)*0.16;group.position.y=group.userData.baseY+bob;if(art)art.rotation.z=Math.sin(t*1.7+seed)*0.035;if(back)back.scale.set(1.05+Math.sin(t*2.8+seed)*0.035,1.06,1);if(shadow)shadow.material.opacity=0.2+Math.sin(t*2.4+seed)*0.05;if(ring){ring.rotation.z+=dt*0.35;ring.material.opacity=(state&&state.aura?0.58:0.32)+Math.sin(t*4+seed)*0.08}}else if(kind==="blastEnded"){var charge=state&&state.charge,stunned=state&&state.stunned;group.position.y=group.userData.baseY+Math.sin(t*8+seed)*(charge?0.06:0.025);group.scale.set(charge?1.12:stunned?0.92:1,stunned?0.88:1,charge?1.18:1);if(art)art.rotation.z=Math.sin(t*(charge?12:3)+seed)*(charge?0.08:0.025);if(shadow)shadow.scale.set(charge?1.25:1,charge?0.75:1,1)}else if(kind==="sphinx"){group.position.y=group.userData.baseY+Math.sin(t*1.5+seed)*0.035;if(art)art.rotation.z=Math.sin(t*1.2+seed)*0.012;if(ring){ring.rotation.z+=dt*0.18;ring.material.opacity=0.34+Math.sin(t*2+seed)*0.08}}group.children.forEach(function(ch){if(ch.name==="assetWisp"&&ch.userData.home){var h=ch.userData.home,s=ch.userData.seed||0;ch.position.x=h.x+Math.sin(t*1.7+s)*0.12;ch.position.y=h.y+Math.sin(t*2.2+s)*0.16;ch.position.z=h.z+Math.cos(t*1.4+s)*0.08;ch.material.opacity=0.12+Math.sin(t*2.5+s)*0.08}})}

var creatureMotionTimer=setInterval(function(){var t=performance.now()/1000,dt=0.033;sphinxes.forEach(function(s){if(s.mesh)animateAssetCreature(s.mesh,t,dt,{})});[sentinel,sentinel2].forEach(function(s){if(s&&s.mesh&&s.mesh.visible!==false)animateAssetCreature(s.mesh,t,dt,{aura:dementorAura})});blastEndedPack.forEach(function(be){if(be&&be.mesh&&be.mesh.visible!==false)animateAssetCreature(be.mesh,t,dt,{charge:be.state==="charge",stunned:be.state==="stunned"})})},33);
...
function addSphinx(r,c,gc,qi,li){var pos=cellToWorld(r,c);var stone=new THREE.MeshStandardMaterial({color:0x806a4f,emissive:0x261b10,emissiveIntensity:0.32,roughness:0.86});var group=makeCreatureStandee(WORLD_IMAGE_SRC.sphinx,2.15,2.85,{kind:"sphinx",baseY:0,shadowX:1.25,shadowZ:0.72,backColor:0x2b2117,backOpacity:0.42,ring:{radius:1.02,tube:0.025,color:0xffd58b,opacity:0.38,y:1.14},core:{geo:new THREE.BoxGeometry(1.35,0.56,1.75),mat:stone,pos:new THREE.Vector3(0,0.5,-0.12),scale:new THREE.Vector3(1,1,1)}});var paws=new THREE.Mesh(new THREE.BoxGeometry(1.8,0.18,0.34),stone);paws.position.set(0,0.18,0.48);paws.castShadow=true;group.add(paws);var glow=new THREE.PointLight(0xffdd8c,1.6,7);glow.position.set(0,1.55,0.25);group.add(glow);group.position.set(pos.x,0,pos.z);var gw=cellToWorld(gc.r,gc.c);group.lookAt(new THREE.Vector3(gw.x,1.4,gw.z));scene.add(group);var barrier=new THREE.Mesh(new THREE.BoxGeometry(CELL*0.9,2.35,0.22),new THREE.MeshStandardMaterial({color:0x78cfff,emissive:0x2588ff,emissiveIntensity:1.3,transparent:true,opacity:0.72,roughness:0.18}));barrier.position.set(gw.x,1.25,gw.z);barrier.castShadow=true;scene.add(barrier);var s={r:r,c:c,gateCell:gc,quizIndex:qi,mesh:group,barrier:barrier,solved:false,linkedGateIdx:li};sphinxes.push(s);quizBarriers.push(s)}
...
function addSentinelObj(s){var robe=new THREE.MeshStandardMaterial({color:0x090b0d,emissive:0x102f36,emissiveIntensity:0.9,roughness:0.9,transparent:true,opacity:0.92});var body=makeCreatureStandee(WORLD_IMAGE_SRC.dementor,1.95,3.3,{kind:"dementor",baseY:1.02,shadowX:0.8,shadowZ:0.5,shadowOpacity:0.26,backColor:0x010304,backOpacity:0.55,ring:{radius:0.78,tube:0.02,color:0x8eeeff,opacity:0.35,y:1.7},core:{geo:new THREE.CapsuleGeometry(0.42,1.55,7,14),mat:robe,pos:new THREE.Vector3(0,1.25,-0.12),scale:new THREE.Vector3(1,1.08,0.72)}});addMistWisps(body,0x8eeeff,7,2.3);body.castShadow=true;var eye=new THREE.PointLight(0xbdf6ff,2.8,9);s.path=[];var h=mapHeight(),w=mapWidth();for(var i=0;i<6;i++){for(var tries=0;tries<100;tries++){var pr=2+Math.floor(Math.random()*(h-4)),pc=2+Math.floor(Math.random()*(w-4));if(!isWallCell(pr,pc)){s.path.push({r:pr,c:pc});break}}}if(s.path.length===0)s.path.push({r:Math.floor(h/2),c:Math.floor(w/2)});var first=cellToWorld(s.path[0].r,s.path[0].c);s.position.set(first.x,1.02,first.z);body.position.copy(s.position);eye.position.set(first.x,1.82,first.z);s.mesh=body;s.light=eye;scene.add(body,eye)}
function addSentinel(){addSentinelObj(sentinel);if(currentDifficulty==="hard")addSentinelObj(sentinel2);}
function addBlastEndedSkrewt(be){var pos=cellToWorld(be.home.r,be.home.c);var carapace=new THREE.MeshStandardMaterial({color:0x3b2015,emissive:0x3b0c04,emissiveIntensity:0.72,roughness:0.64});var group=makeCreatureStandee(WORLD_IMAGE_SRC.blastEnded,2.0,1.72,{kind:"blastEnded",baseY:0.26,shadowX:1.05,shadowZ:0.58,shadowOpacity:0.36,backColor:0x1f0c06,backOpacity:0.44,core:{geo:new THREE.SphereGeometry(0.74,18,12),mat:carapace,pos:new THREE.Vector3(0,0.62,-0.1),scale:new THREE.Vector3(1.55,0.58,0.9)}});var tail=new THREE.Mesh(new THREE.ConeGeometry(0.22,1.05,14),new THREE.MeshStandardMaterial({color:0xd44718,emissive:0xa81808,emissiveIntensity:1.15,roughness:0.45}));tail.position.set(0,0.78,0.82);tail.rotation.x=Math.PI/2;tail.name="skrewtTail";tail.castShadow=true;group.add(tail);var ember=new THREE.PointLight(0xff5d28,1.8,5);ember.position.set(0,0.92,0.78);group.add(ember);group.position.set(pos.x,0.26,pos.z);scene.add(group);var light=new THREE.PointLight(0xff5d28,1.2,5);light.position.set(pos.x,1,pos.z);scene.add(light);be.mesh=group;be.light=light;be.position.set(pos.x,0.26,pos.z);be.state="patrol";be.patrolPhase=Math.random()*Math.PI*2;be.hitTimer=0;be.listenTimer=0}
...
if(btnScroll)btnScroll.addEventListener("pointerdown",function(e){e.preventDefault();e.stopPropagation();useScroll()});
