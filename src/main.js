...
var masterGain=null,reverbGain=null;
function audioOut(){return masterGain||(audioCtx&&audioCtx.destination)}
function makeImpulse(ctx,seconds,decay){var rate=ctx.sampleRate,len=Math.max(1,Math.floor(rate*seconds)),buf=ctx.createBuffer(2,len,rate);for(var ch=0;ch<2;ch++){var d=buf.getChannelData(ch);for(var i=0;i<len;i++){d[i]=(Math.random()*2-1)*Math.pow(1-i/len,decay)}}return buf}
function buildMasterBus(){if(!audioCtx||masterGain)return;try{masterGain=audioCtx.createGain();masterGain.gain.value=0.85;var comp=audioCtx.createDynamicsCompressor();comp.threshold.value=-16;comp.knee.value=22;comp.ratio.value=4;comp.attack.value=0.004;comp.release.value=0.2;masterGain.connect(comp);try{var conv=audioCtx.createConvolver();conv.buffer=makeImpulse(audioCtx,2.4,2.6);reverbGain=audioCtx.createGain();reverbGain.gain.value=0.16;masterGain.connect(conv);conv.connect(reverbGain);reverbGain.connect(comp)}catch(e){}comp.connect(audioOut())}catch(e){masterGain=null}}
...
function ensureAudio(){var E=window.AudioContext||window.webkitAudioContext;if(!E)return;if(!audioCtx)audioCtx=new E();if(audioCtx.state==="suspended")audioCtx.resume();buildMasterBus();if(!tinnitusOsc&&audioCtx){try{tinnitusOsc=audioCtx.createOscillator();tinnitusGain=audioCtx.createGain();tinnitusOsc.type="sine";tinnitusOsc.frequency.value=3400;tinnitusGain.gain.value=0;tinnitusOsc.connect(tinnitusGain).connect(audioOut());tinnitusOsc.start();wailOsc=audioCtx.createOscillator();wailGain=audioCtx.createGain();wailOsc.type="sawtooth";wailOsc.frequency.value=180;wailGain.gain.value=0;wailOsc.connect(wailGain).connect(audioOut());wailOsc.start();}catch(e){tinnitusOsc=null;wailOsc=null;}}}
function startAmbientDrone(){if(!audioCtx||ambientDrone)return;var low=audioCtx.createOscillator(),high=audioCtx.createOscillator(),gain=audioCtx.createGain(),filter=audioCtx.createBiquadFilter();low.type="sine";high.type="triangle";low.frequency.value=54;high.frequency.value=108;filter.type="lowpass";filter.frequency.value=380;gain.gain.value=0.0001;low.connect(filter);high.connect(filter);filter.connect(gain).connect(audioOut());low.start();high.start();gain.gain.setTargetAtTime(0.012,audioCtx.currentTime,0.8);ambientDrone={low:low,high:high,gain:gain,filter:filter}}
