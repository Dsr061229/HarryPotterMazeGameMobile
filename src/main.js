...
var heroConfig = {
  harry: {
    ...
    weapon:function(){
      ...
      blastEndedPack.forEach(function(be){if(be.mesh&&be.state!=="stunned"){var bep=new THREE.Vector3(player.position.x,0.62,player.position.z);if(be.position.distanceTo(bep)<4){hit=true;var dirBE=new THREE.Vector3(be.position.x-player.position.x,0,be.position.z-player.position.z).normalize().multiplyScalar(CELL*4);be.position.add(dirBE);be.mesh.position.copy(be.position);if(be.light)be.light.position.set(be.position.x,1,be.position.z);be.state="stunned";be.stunTimer=GAME_BALANCE.blastEnded.stunSeconds;}}});
...
if (restartButton) restartButton.addEventListener("click", function () { resultPanel.classList.add("hidden"); overlay.classList.remove("hidden"); resetLobby(); checkOrientation(); setMessage("选择你的难度和勇士，然后再次踏入迷宫！",2); });
...
function addSentinelObj(s){var body=new THREE.Mesh(new THREE.CapsuleGeometry(0.52,1.4,7,14),new THREE.MeshStandardMaterial({color:0x08070a,emissive:0x28304a,emissiveIntensity:0.7,roughness:0.55}));body.castShadow=true;var eye=new THREE.PointLight(0xbfd7ff,2.8,9);s.path=[];var h=mapHeight(),w=mapWidth();for(var i=0;i<6;i++){for(var tries=0;tries<100;tries++){var pr=2+Math.floor(Math.random()*(h-4)),pc=2+Math.floor(Math.random()*(w-4));if(!isWallCell(pr,pc)){s.path.push({r:pr,c:pc});break}}}if(s.path.length===0)s.path.push({r:Math.floor(h/2),c:Math.floor(w/2)});var first=cellToWorld(s.path[0].r,s.path[0].c);s.position.set(first.x,1.02,first.z);body.position.copy(s.position);eye.position.set(first.x,1.7,first.z);s.mesh=body;s.light=eye;scene.add(body,eye)}
function addSentinel(){addSentinelObj(sentinel);if(currentDifficulty==="hard")addSentinelObj(sentinel2);}
function addBlastEndedSkrewt(be){var pos=cellToWorld(be.home.r,be.home.c);var group=new THREE.Group();var shell=new THREE.Mesh(new THREE.SphereGeometry(0.72,18,12),new THREE.MeshStandardMaterial({color:0x3b2318,emissive:0x351008,roughness:0.64}));shell.scale.set(1.35,0.62,0.82);var tail=new THREE.Mesh(new THREE.CylinderGeometry(0,0.22,1,12),new THREE.MeshStandardMaterial({color:0xbb3b18,emissive:0x8a1208,roughness:0.45}));tail.position.z=0.92;tail.rotation.x=Math.PI/2;group.add(shell,tail);group.position.set(pos.x,0.62,pos.z);scene.add(group);var light=new THREE.PointLight(0xff5d28,1.2,5);light.position.set(pos.x,1,pos.z);scene.add(light);be.mesh=group;be.light=light;be.position.set(pos.x,0.62,pos.z);be.state="patrol";be.patrolPhase=Math.random()*Math.PI*2;be.hitTimer=0;be.listenTimer=0}
