...
var heroConfig = {
  harry: {
    ...
    weapon:function(){
      ...
      if(sentinel.position.distanceTo(pf)>4.5)return false;
      ...
      playChord([262,330,392],0.22,0.06);speakSpell("除你武器！");setMessage("格兰芬多宝剑斩击！摄魂怪被击退。",2);
      return true;
    }
  },
  cedric: {
  ...
  viktor: {
    ...
    weapon:function(){
      ...
      sentinel.forcedChaseTimer=0;sentinel.banished=false;sentinel._frozen=0;
      sentinel.position.set(999,0,999);sentinel.mesh.position.copy(sentinel.position);
  ...
  fleur: {
    ...
    weapon:function(){
      ...
      var d=quizData[currentQuiz.quizIndex%quizData.length];
      var wrongs=[];for(var i=0;i<d.options.length;i++){if(i!==d.answer)wrongs.push(i);}
      var btns=quizOptionsEl.querySelectorAll("button");
      for(var k=0;k<Math.min(2,wrongs.length);k++){
        var wi=wrongs[k];if(btns[wi]){btns[wi].style.opacity="0.2";btns[wi].style.textDecoration="line-through";btns[wi].disabled=true;}
      }
...
if (startButton) startButton.addEventListener("click", function () {
  ...
  overlay.classList.add("hidden");authPanel.classList.remove("hidden");
});
...
if(rulesOk)rulesOk.addEventListener("click",function(){rulesPanel.classList.add("hidden");authPanel.classList.add("hidden");checkOrientation();startGame()});
...
function generateMaze(){
  ...
  for(var ga=0;ga<30;ga++){grid=[];for(var r=0;r<SIZE;r++){grid[r]=[];for(var c=0;c<SIZE;c++)grid[r][c]="#"}
    ...
    carve(1,1);
    var maxO=25+Math.floor(Math.random()*16),totalO=0,ol=[]; // 收集候选墙
...
var scene=new THREE.Scene();scene.background=new THREE.Color(0x1a2a1e);scene.fog=new THREE.Fog(0x1a2a1e,8,36);
...
function addTriwizardCup(){var pos=cellToWorld(exitCell.r,exitCell.c);var cup=new THREE.Group();var gold=new THREE.MeshStandardMaterial({color:0xffd675,metalness:0.72,roughness:0.24,emissive:0x4a3510,emissiveIntensity:0.4});var blue=new THREE.MeshBasicMaterial({color:0x9ee9ff,transparent:true,opacity:0.85});var bowl=new THREE.Mesh(new THREE.CylinderGeometry(0.9,0.48,0.72,32),gold);var stem=new THREE.Mesh(new THREE.CylinderGeometry(0.18,0.24,0.82,24),gold);var foot=new THREE.Mesh(new THREE.CylinderGeometry(0.72,0.42,0.2,32),gold);var lip=new THREE.Mesh(new THREE.TorusGeometry(0.9,0.06,10,36),gold);var lh=new THREE.Mesh(new THREE.TorusGeometry(0.48,0.045,10,26,Math.PI),gold);var rh=lh.clone();var flame=new THREE.Mesh(new THREE.ConeGeometry(0.42,0.9,24),blue);bowl.position.y=1.45;stem.position.y=0.75;foot.position.y=0.26;lip.position.y=1.83;lh.position.set(-0.83,1.5,0);lh.rotation.z=Math.PI/2;rh.position.set(0.83,1.5,0);rh.rotation.z=-Math.PI/2;flame.position.y=2.36;flame.name="cupFlame";cup.add(bowl,stem,foot,lip,lh,rh,flame);cup.position.set(pos.x,0,pos.z);cup.name="triwizardCup";scene.add(cup);var cl=new THREE.PointLight(0x9ee9ff,7,18);cl.name="cupLight";cl.position.set(pos.x,2.2,pos.z);scene.add(cl);var pg=new THREE.SphereGeometry(0.05,8,8);for(var i=0;i<16;i++){var p=new THREE.Mesh(pg,blue.clone());p.position.set(pos.x,2+Math.random()*2.2,pos.z);scene.add(p);exitParticles.push({mesh:p,seed:Math.random()*100,radius:0.5+Math.random()*1.2,height:2+Math.random()*2.0})}}
...
function animate(now){var dt=Math.min((now-lastTime)/1000,0.05);lastTime=now;var t=now/1000;if(weaponCooldown>0)weaponCooldown-=dt;updateSceneMotion(t,dt);if(gameState==="playing"){updatePlayer(dt);updateSentinel(dt,t);updateBlastEnded(dt,t);updateGameRules(dt,t);updateInteractionPrompt();updateExitAudio(t);updateStateAudio(t)}else if(gameState==="ceremony"){updateCeremony(dt,t);updatePlayer(dt);updateSentinel(dt,t);updateStateAudio(t)}else{updateSentinel(dt,t)}lumos.intensity=8.0+wandPower*8.0+Math.sin(t*0.012)*0.3;lumos.distance=(14+wandPower*16)*heroLightMult;lumos.angle=Math.PI/(4.5*Math.sqrt(heroLightMult));updateCamera(t);updateJoystick();renderer.render(scene,camera);requestAnimationFrame(animate)}
...
function isDeadEndCell(r,c){if(isWallCell(r,c))return false;var openCnt=0,ds=[[1,0],[-1,0],[0,1],[0,-1],[2,0],[-2,0],[0,2],[0,-2]];var range=1+(heroDeadEndR||0);for(var d=0;d<(range===1?4:8);d++){var nr=r+ds[d][0],nc=c+ds[d][1];if(!isSolid(nr,nc))openCnt++}return openCnt<=1}
...
(function init(){if(typeof THREE==='undefined'){alert('Three.js库未能加载。请检查网络后刷新。');return}try{generateMaze();initWorld();checkOrientation();requestAnimationFrame(animate);authPanel.classList.add("hidden")}catch(e){alert('初始化失败: '+(e.message||e)+'\n请刷新页面重试。')}})();
