...
// ===== 大厅交互 =====
function resetLobby(){currentDifficulty="medium";currentHero=null;if(diffTabs){diffTabs.querySelectorAll(".diff-btn").forEach(function(b){b.classList.remove("active");if(b.dataset.diff==="medium")b.classList.add("active")})}if(heroCards){heroCards.querySelectorAll(".hero-card").forEach(function(c){c.classList.remove("selected")})}}
if(diffTabs){
...
function finishAuth(uid){isAdmin=false;currentUser=uid;btnAdmin.classList.add("hidden");authPanel.classList.add("hidden");overlay.classList.remove("hidden");resetLobby();setMessage("欢迎回来，"+uid+"！选择勇士踏入迷宫。",2)}
